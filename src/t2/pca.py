...
class ExpressionPCA:
    """Incremental PCA on the T2 MERFISH panel (498/500 genes). Independent of T1's 32k PCA."""

    def __init__(self, n_components: int = 32, batch_size: int = 256):
    ...
    def encode_residual(
        ...
        """Encode clip(x − Δ) so CFM learns residual transport, not the global shift."""
        self._check()
        delta = np.asarray(delta, dtype=np.float32).ravel()
        zs = []
        for start in range(0, adata.n_obs, self.batch_size):
            sl = slice(start, min(start + self.batch_size, adata.n_obs))
            x = np.clip(to_dense(adata[sl].X) - delta, clip_min, None)
            zs.append(self.encode(x))
        return np.concatenate(zs, axis=0)

    def decode(self, z: np.ndarray, clip_min: float | None = 0.0) -> np.ndarray:
        self._check()
